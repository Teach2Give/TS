const arr1: string[] = ["apple", "mango", "banana"];
const arr2: number[] = [1, 2, 3];
const arr3: boolean[] = [false, true];

// opening the type definition we see
interface Array<T> {
  // T represents the type of the array elements
  pop(): T | undefined;
  push(...items: T[]): number;
  // Other methods that work generically over T
}
...
const pair = makePair<string, number>("apple", 5); // ['apple', 5]
const pair2 = makePair<boolean, string>(true, "banana"); // [true, 'banana']
...
const stringBox = new Box<string>("apple");
